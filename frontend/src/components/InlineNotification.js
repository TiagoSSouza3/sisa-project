...
import '../styles/inline-notification.css';

const InlineNotification = ({ message, type = 'success', duration = 6000, onClose }) => {
  ...
  return (
    <div className={`inline-notification inline-notification--${type} ${isExiting ? 'inline-notification--exiting' : ''}`}>
      <div className="inline-notification__icon">
        {getIcon()}
      </div>
      <div className="inline-notification__content">
        <p className="inline-notification__message">{message}</p>
      </div>
    </div>
  );
};
