const db = require("../config/firebase");
const { firebaseCollections } = require("../enums/firebaseCollections");
const { applyWhere } = require("./utilsService.js");
const documentVersionsRef = db.collection(
  firebaseCollections.DOCUMENT_VERSIONS
);

exports.getAll = async (options = {}) => {
  const snapshot = await documentVersionsRef.get();
  if (snapshot.empty) return [];

  let data = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));

  if (options.where) {
    data = applyWhere(data, options.where);
  }

  if (options.order && Array.isArray(options.order) && options.order.length) {
    const [field, direction] = options.order[0];
    data.sort((a, b) => {
      const av = a[field];
      const bv = b[field];
      if (av === bv) return 0;
      if (direction === "DESC") {
        return av < bv ? 1 : -1;
      }
      return av > bv ? -1 : 1;
    });
  }

  return data;
};

exports.findPk = async (id, _options = {}) => {
  const snapshot = await documentVersionsRef.doc(id).get();
  if (!snapshot.exists) return null;
  return { id: snapshot.id, ...snapshot.data() };
};

exports.create = async (data) => {
  const docRef = await documentVersionsRef.add(data);
  const snapshot = await docRef.get();
  return { id: snapshot.id, ...snapshot.data() };
};

exports.destroy = async (idOrWhere) => {
  if (typeof idOrWhere === "string") {
    await documentVersionsRef.doc(idOrWhere).delete();
    return 1;
  }

  const snapshot = await documentVersionsRef.get();
  if (snapshot.empty) return 0;

  const all = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  const toDelete = applyWhere(all, idOrWhere || {});

  await Promise.all(
    toDelete.map((item) => documentVersionsRef.doc(item.id).delete())
  );
  return toDelete.length;
};

exports.findOne = async (options = {}) => {
  const list = await exports.getAll(options);
  return list[0] || null;
};

exports.update = async (instanceOrId, data) => {
  const id =
    typeof instanceOrId === "string"
      ? instanceOrId
      : instanceOrId && instanceOrId.id;

  if (!id) {
    throw new Error(
      "documentVersionService.update requires an id or instance with id"
    );
  }

  await documentVersionsRef.doc(id).set({ id, ...data }, { merge: true });
  const snapshot = await documentVersionsRef.doc(id).get();
  return { id: snapshot.id, ...snapshot.data() };
};